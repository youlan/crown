...
export const getSearchEntities: ICrudSearchAction<IRequestPoint> = (query, page, size, sort) => ({
  ...
  payload: axios.get<IRequestPoint>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
...
export const getEntities: ICrudGetAllAction<IRequestPoint> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IRequestPoint>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};
